...
def show_all_images(
    ...
    x, y_bin, *_ = dataset[sample_img]

    if x.ndim == 4:          # (1,T,H,W)
        input_frames = x[0]  # (T,H,W) for plotting
        input_img = x.unsqueeze(0).to(device)   # (1,1,T,H,W) for model
    elif x.ndim == 3:        # (T,H,W)
        input_frames = x
        input_img = x.unsqueeze(0).unsqueeze(0).to(device)  # (1,1,T,H,W)
    else:
        raise ValueError(f"Unexpected x shape: {tuple(x.shape)}")

    target_img = y_bin.cpu()
    if target_img.ndim == 3:
        target_img = target_img.squeeze(0)

    with torch.no_grad():
        raw_pred = model(input_img).detach().cpu()   # [1,2,H,W]

    pred_scores = torch.softmax(raw_pred[0], dim=0)[1]
    pred_mask   = (pred_scores >= water_threshold).float()
    ...
    cmap_input  = ListedColormap(['black', 'palegoldenrod', 'navy'])
    cmap_target = ListedColormap(['palegoldenrod', 'navy'])
    cmap_pred   = ListedColormap(['palegoldenrod', 'navy'])
    cmap_diff   = ListedColormap(['white', 'blue', 'yellow', 'red'])

    # --- drop padded frames for DISPLAY only ---
    keep = (input_frames != pad_value).any(dim=(1, 2))
    disp_frames = input_frames[keep]
    C = disp_frames.shape[0]
    ...
    for i in range(C):
        r = i // n_cols
        c = i % n_cols
        ax[r, c].imshow(disp_frames[i].cpu(), cmap=cmap_input, vmin=0, vmax=2)
        ax[r, c].set_title(f'Input {i}', fontsize=10)
        ax[r, c].axis('off')
    ...
    # use last NON-padded displayed frame for erosion/deposition (fallback to last frame)
    frame_for_bar = disp_frames[-1].cpu() if C > 0 else input_frames[-1].cpu()
    prev_water = (frame_for_bar == 2).float()   # 2 is water in your input frames
    real_erosion_deposition = get_erosion_deposition(prev_water, target_img, nonwater, water, pixel_size)
    pred_erosion_deposition = get_erosion_deposition(prev_water, pred_mask,  nonwater, water, pixel_size)
