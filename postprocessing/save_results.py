...
#     if physics:
#         if alpha_er == None or alpha_dep == None:
#             raise Exception(f'Attention! you set `physics` = {physics} but `alpha_er`= {alpha_er} and `alpha_dep`= {alpha_dep}.\n\
# You must set both `alpha_er` and `alpha_dep`.')
#         else:
...
def save_losses_metrics(
    train_losses, val_losses, metrics, spatial_temporal, model, month_dataset,
    init_hid_dim=None, T=None, learning_rate=None, step_size=None, gamma=None,
    batch_size=None, num_epochs=None, water_threshold=None,
    physics=False, alpha_er=None, alpha_dep=None,
    dir_output=r"model\losses_metrics",
):
    os.makedirs(dir_output, exist_ok=True)
    model_name = _model_name(model)

    file_name = f"{model_name}{spatial_temporal}_month{month_dataset}_base{init_hid_dim}_T{T}_lr{learning_rate}"
    if (step_size is not None) and (gamma is not None):
        file_name += f"{step_size}step_{gamma}gamma_"
    file_name += f"{batch_size}batch_{num_epochs}epochs_{water_threshold}wthr"

    if physics:
        if (alpha_er is None) or (alpha_dep is None):
            raise ValueError("physics=True requires alpha_er and alpha_dep.")
        file_name += f"{alpha_er}aer{alpha_dep}adep"
...
def save_model_path(
    model, spatial_temporal, loss_recall,
    month_dataset=None, init_hid_dim=None, T=None,
    learning_rate=None, step_size=None, gamma=None,
    batch_size=None, num_epochs=None, water_threshold=None,
    physics=False, alpha_er=None, alpha_dep=None,
    dir_output=r"model\models_trained",
):
    os.makedirs(dir_output, exist_ok=True)
    best_model = copy.deepcopy(model)

    model_name = _model_name(model)
    file_name = f"{model_name}{loss_recall}{spatial_temporal}month{month_dataset}_base{init_hid_dim}_T{T}_lr{learning_rate}"
    if (step_size is not None) and (gamma is not None):
        file_name += f"{step_size}step_{gamma}gamma_"
    file_name += f"{batch_size}batch_{num_epochs}epochs_{water_threshold}wthr"
...
def load_model(model, save_path, device):
    state = torch.load(save_path, map_location=torch.device(device))
